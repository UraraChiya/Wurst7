...
public final class TpsComponent extends Component
{
					   ...
					   float partialTicks)
	{
		...
		if (lastTickSecond < 6.0)
			fr.draw(matrixStack, String.format("TPS: %.2f", tps), x1, y1, tps > 19.0 ? gui.getTxtColor() : Color.RED.getRGB());
		else
			fr.draw(matrixStack, String.format("TPS: < %.2f", 20.0f / lastTickSecond), x1, y1, Color.RED.getRGB());
